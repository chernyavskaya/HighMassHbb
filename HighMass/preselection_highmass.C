...
int preselection(Int_t nJets, Float_t Jet_pt[300], Float_t Jet_eta[300], Float_t Jet_phi[300], Float_t Jet_mass[300], Float_t Jet_btagCSV[300], Int_t Jet_id[300],Int_t Jet_puId[300], Int_t& btag_max1_number, Int_t& btag_max2_number, Int_t& eta_max1_number, Int_t& eta_max2_number, Float_t trigger, TLorentzVector& Bjet1,TLorentzVector& Bjet2, TLorentzVector& Qjet1, TLorentzVector& Qjet2,TLorentzVector& qq,  std::vector<TLorentzVector> &jets_pv, std::vector<float> &jets_btag, std::vector<float> &jets_pt, int& good_jets, std::vector<int> &jets_indices, float met_pt, float met_phi, TLorentzVector &met, std::vector<TLorentzVector> &FSRjet){
	...
	int not_pass=0;
      ...
      btag_max2_number =   max_element( jets_btag_times_pt.begin(), jets_btag_times_pt.end()) -  jets_btag_times_pt.begin();
		...
		double met_dphi1 = TMath::Abs(met_phi - Bjet1.Phi());
		double met_dphi2 = TMath::Abs(met_phi - Bjet2.Phi());
		if (met_dphi1 < met_dphi2) met.SetPtEtaPhiM(met_pt * TMath::Cos(std::min(met_dphi1,met_dphi2)),Bjet1.Eta(),Bjet1.Phi(),0);
		else  met.SetPtEtaPhiM(met_pt * TMath::Cos(std::min(met_dphi1,met_dphi2)),Bjet2.Eta(),Bjet2.Phi(),0);
		...
		if (!(Mbb_met_fsr>250)) {not_pass=-8; cut_count++;}
