...
class BiasStudy:
    ...
    def __init__(self, fname="", ws_name="Xbb_workspace", read_dir="/afs/cern.ch/work/n/nchernya/HighMassHbb/bias/firstStep/", save_dir="/afs/cern.ch/work/n/nchernya/HighMassHbb/bias/output/", save_ext=['png'], blind_plot=False):
        self.fname = fname
    ...
    def plot(self, data=None, hdata=None, pdfs=[], params=[], res=[], probs=[], npars=[], legs=[], add_ratio=False, title="", header=""):
        ...
        frame.GetYaxis().SetTitle("Events / "+str(frame.GetXaxis().GetBinWidth(10))+" GeV")
        ...
        #    h_rebinned = self.data.createHistogram("h_"+data_name+"_rebinned", self.x, RooFit.Binning( int((self.x.getMax()-self.x.getMin())/5.0) , self.x.getMin(), self.x.getMax()) )
         #   data_rebinned = ROOT.RooDataHist(data_name+"_rebinned","", ROOT.RooArgList(self.x), h_rebinned, 1.0)
           ...
           ## self.plot(data=data_rebinned, pdfs=pdfs, probs=probs, npars=npars, legs=legs, title=pdf_name+"_"+data_name+("_sgnInjected" if add_fake_sgn>0. else ""), header="F-test: "+pdf_name+(", signal injected" if add_fake_sgn>0. else ""))
            self.plot(data=self.data, pdfs=pdfs, probs=probs, npars=npars, legs=legs, title=pdf_name+"_"+data_name+("_sgnInjected" if add_fake_sgn>0. else ""), header="F-test: "+pdf_name+(", signal injected" if add_fake_sgn>0. else ""))
...
bs = BiasStudy(fname=cfg_fname, 
            #  hist_name="Mbb_met_fsr", 
               ws_name="Xbb_workspace",
					read_dir="/afs/cern.ch/work/n/nchernya/HighMassHbb/bias/firstStep/", 
               save_dir="/afs/cern.ch/work/n/nchernya/HighMassHbb/bias/plots/", 
               save_ext=['png', 'pdf'], 
               blind_plot=False
               )

bs.doFTest(data_name="data_obs", test_pdfs=test_pdfs, parameter_set="default", add_fake_sgn=0, fake_sgn=cfg_sgn_name)
