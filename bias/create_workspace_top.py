...
class XbbFactory:
    ...
    def plot(self, data=None, pdfs=[], res=None, add_pulls=False, legs=[], ran="", n_par=1, title="", header=""):
        ...
        #    frame2.GetYaxis().SetTitle("Pulls")
            ...
            #    hresid = frame.pullHist()
              ...
              #  hresid.GetYaxis().SetRangeUser(-1000,1000)
                h_Data = data.createHistogram("_resid", self.x, RooFit.Binning(int((self.x.getMax()-self.x.getMin())/20.0)  , self.x.getMin(), self.x.getMax()) )
                ...
                pdf = pdfs[0]
                h_Fit = pdf.createHistogram("_pdf_for_resid", self.x, RooFit.Binning( int((self.x.getMax()-self.x.getMin())/20.0), self.x.getMin(), self.x.getMax()) )
        ...
        for ext in self.save_ext:
            c1.SaveAs("DONTLOOT_"+self.save_dir+self.ws_name+"_"+self.get_save_name()+"_"+title+"_npar"+str(n_par)+"."+ext)
   ...
   #     data_norm = h.Integral()
    ...
    def test_datafit(self, pdf_names=['dijet'], rebin_factor=1.0):
     ...
     #       [pdf_bkg, param_bkg] = generate_pdf(self.x, pdf_name=pdf_name, n_param=FTestCfg_data[pdf_name]['MaxOrder'][pdf_range], n_iter=0, gcs=gcs, mass_range=pdf_range, parameter_set="top", is_data=True)
            [pdf_bkg, param_bkg] = generate_pdf(self.x, pdf_name=pdf_name, n_param=FTestCfg_data[pdf_name]['MaxOrder'][pdf_range], n_iter=0, gcs=gcs, mass_range=pdf_range, parameter_set="default", is_data=True)
    ...
    #        self.add_sgn_to_ws(sgn_name=sgn, rebin_factor=-1, set_param_const=True, spin_symmetric=False)
     ...
     #   if debug_sgn:            
      ...
      #  self.add_bkg_to_ws(pdf_names=pdf_names, rebin_factor=-1, set_param_const=False)
        ...
        self.w.Print()
        self.w.writeToFile("DONTOPEN_"+self.save_dir+self.ws_name+"_"+self.get_save_name()+".root")
...
cfg_cat_btag = ""
cfg_cat_kin = "" 
cfg_name = argv[1] if len(argv)>=2 else "hMbb_met_fsr_sg"
